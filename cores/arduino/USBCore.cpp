...
extern const DeviceDescriptor USB_DeviceDescriptorC PROGMEM;
...
uint16_t _initEndpointsSize[USB_ENDPOINTS] =
{
	USB_CONTROL_EP_SIZE,		// Control Endpoint
#ifdef CDC_ENABLED
	USB_CDC_NOTIFICATION_EP_SIZE,	// CDC_ENDPOINT_ACM
	USB_CDC_DATA_EP_SIZE,		// CDC_ENDPOINT_OUT
	USB_CDC_DATA_EP_SIZE,		// CDC_ENDPOINT_IN
#endif
	// Following endpoints are automatically initialized to 0
};

uint8_t _initEndpointsBanks[USB_ENDPOINTS] =
{
	USB_CONTROL_EP_BANKS,		// Control Endpoint
#ifdef CDC_ENABLED
	USB_CDC_NOTIFICATION_EP_BANKS,	// CDC_ENDPOINT_ACM
	USB_CDC_DATA_EP_BANKS,		// CDC_ENDPOINT_OUT
	USB_CDC_DATA_EP_BANKS,		// CDC_ENDPOINT_IN
#endif
	// Following endpoints are automatically initialized to 0
};
...
const DeviceDescriptor USB_DeviceDescriptor =
	D_DEVICE(0x00,0x00,0x00,USB_CONTROL_EP_SIZE,USB_VID,USB_PID,0x100,IMANUFACTURER,IPRODUCT,ISERIAL,1);

const DeviceDescriptor USB_DeviceDescriptorB =
	D_DEVICE(0xEF,0x02,0x01,USB_CONTROL_EP_SIZE,USB_VID,USB_PID,0x100,IMANUFACTURER,IPRODUCT,ISERIAL,1);

const DeviceDescriptor USB_DeviceDescriptorC =
	D_DEVICE(0x02,0x00,0x00,USB_CONTROL_EP_SIZE,USB_VID,USB_PID,0x100,IMANUFACTURER,IPRODUCT,ISERIAL,1);
...
//	Space in send EP
int USB_SendSpace(u8 ep)
{
	LockEP lock(ep);
	if (!ReadWriteAllowed())
		return 0;
	return (int)(_initEndpointsSize[(ep & 7)] - FifoByteCount());
}
...
static
void InitEP(u8 index)
{
	uint16_t size = _initEndpointsSize[index];
	if (size == 0) {
		size = USB_DEFAULT_EP_SIZE;
	}
	...
	uint8_t banks = _initEndpointsBanks[index];
	if (banks == 0) {
		banks = USB_DEFAULT_EP_BANKS;
	}
	banks--;
	
	UENUM = index;
	UECONX = (1<<EPEN);
	UECFG0X = _initEndpoints[index];
	UECFG1X = ( (sizeMask << 4) | (banks << 2) | (1 << 1) );
}
...
bool SendControl(u8 d)
{
	if (_cmark < _cend)
	{
		if (!WaitForINOrOUT())
			return false;
		Send8(d);
		if (!((_cmark + 1) & (_initEndpointsSize[0] - 1)))
...
bool SendDescriptor(USBSetup& setup)
{
	...
	if (USB_DEVICE_DESCRIPTOR_TYPE == t)
	{
		...
		desc_addr = (const u8*)&USB_DeviceDescriptorC;
#else
...
void USB_Flush(u8 ep)
{
	...
	// TODO Comment out the following line to work with Atmel ASF USB CDC-ACM hosts
...
ISR(USB_GEN_vect)
{
	...
	if (udint & (1<<SOFI))
	{
		tickReset();					// for supporting auto-reset
#ifdef CDC_ENABLED
